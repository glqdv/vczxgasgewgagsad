...
function submitform(action, url, postdata) {
    ...
    switch (action) {
        ...
        case 'delete':
            ...
            action = 'Are you sure you want to ' + action + ' it?';
			$(".fixed-table-loading.table.table-bordered.table-hover").css({"display":"block"});
            if (! confirm(action)) return;
			console.log("del :",postdata)
            postsubmit = true;
        case 'add':
        ...
        case 'edit':
            $.ajax({
                ...
                success: function (data) {
                    ...
                    alert(res.msg);
					$(".fixed-table-loading.table.table-bordered.table-hover").css({"display":"none"});
                    if (res.status) {
                        if (postsubmit) {
							document.location.reload();
						}else{
							// history.back(-1);
							document.location.href = "/index/hostlist";
						}
